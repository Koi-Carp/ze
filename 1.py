...
class Spider(Spider):
    ...
    def searchContent(self, key, quick, pg="1"):
        ...
        # 使用更精确的选择器，并避免重复
        videos = []
        seen_ids = set()  # 用于去重的集合
        
        # 尝试查找主要的搜索结果区域
        module_items = doc(".module-items")
        
        # 如果找到模块，只在第一个模块中获取数据
        if module_items:
            main_module = module_items.first()
            items = main_module.find(".module-item")
            
            for item in items.items():
                cover = item.find(".module-item-cover")
                if cover:
                    a = cover("a")
                    vod_id = a.attr('href')
                    ...
                    videos.append({
                        'vod_id': vod_id,
                        'vod_name': a.attr('title'),
                        'vod_pic': self.getProxyUrl() + "&url=" + cover("img").attr("data-cover"),
                        'vod_remark': item.find(".module-item-caption").text() or item.find(".module-item-ru").text(),
                    })
        else:
            # 备用方案：直接查找.module-item元素
